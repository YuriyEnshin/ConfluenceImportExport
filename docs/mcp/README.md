...
Файл [`agent-instructions.md`](agent-instructions.md) — короткий гайд **на
английском, для агента** (не для оператора): что делает сервер, как
работает песочница, как выбирать между инструментами, и (главное)
полные сценарии 2-way и 3-way merge при конфликтах.

Этот файл доставляется агенту **двумя путями**:

1. **Автоматически.** При старте MCP-сервер встраивает его в поле
   `InitializeResult.Instructions` MCP-протокола. Большинство клиентов
   (Claude Code, Claude Desktop, Cursor) подмешивают это в системный
   промпт агента — никакого ручного действия не требуется. Файл
   обновляется вместе с релизом сервера.
2. **Вручную (опционально).** Если хочется, чтобы агент знал правила
   *до* подключения сервера (или вместе с другим контекстом проекта) —
   скопируй содержимое в `CLAUDE.md`, `.cursorrules`, system-prompt
   ChatGPT и т.п. Это удобно, например, в commit-instructions
   репозитория, где живёт зеркало Confluence-папок.

Оба пути совместимы: дублирующиеся инструкции для агента не вредны,
а в случае рассинхронизации (например, ты вкатил старый файл в
правила, а сервер уже новый) — ServerInstructions перевешивают, так
как приезжают ближе к моменту использования.
...
При этом `success: true` **не** означает «всё синхронизировано»: если часть
вложений выгрузить не удалось, они перечисляются в `report.failedAttachments`
(страница, имя файла, причина), поднимают `report.hasIssues` и попадают в
`summary` (`… ; 1 attachment(s) failed`). Точно так же страницы, локальные
изменения которых не удалось применить на сервере (отложенное переименование
или перемещение, ошибка записи), перечисляются в `report.unappliedPages`
с причиной и подсказкой, поднимают `report.hasIssues` и попадают в `summary`
(`… ; 1 unapplied`). Агент обязан проверить эти поля, прежде чем сообщать
пользователю, что зеркало актуально.
