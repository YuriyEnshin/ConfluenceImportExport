...
public class HttpClientConfluenceApiClient : IConfluenceApiClient
{
    ...
    // Attachment media-type plumbing lives in ConfluenceApiHelpers, shared
    // with the Cloud client (both upload through the same v1 multipart shape).
    private static void ApplyAttachmentContentType(ByteArrayContent part, string fileName, string? preferredMediaType) =>
        ConfluenceApiHelpers.ApplyAttachmentContentType(part, fileName, preferredMediaType);
