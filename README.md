...
1. **Сравнение дат** (всегда) — сопоставляет дату последней модификации серверной страницы (`version.when`) с датой изменения локальной папки (переименование/перемещение) или файла `index.html` (контент). Достоверность: средняя.

2. **Анализ истории версий** (с `--detect-source`) — для переименований ищет прежний заголовок в истории версий страницы, для перемещений ищет прежнего родителя в ancestors исторических версий. Достоверность: высокая.
