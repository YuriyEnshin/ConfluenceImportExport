...
- Работа с несколькими пространствами: пространство страницы сохраняется в маркере и берётся с сервера; деревья из разных пространств можно держать рядом и синхронизировать вместе (`--multi-tree` для upload). Непреднамеренный перенос поддерева в чужое пространство отклоняется и попадает в отчёт
...
- заголовок `Модуль "Провайдеры"` на Windows даст папку `Модуль _Провайдеры_`, а на Linux — `Модуль "Провайдеры"` (такое имя папки на Windows создать нельзя);
- прямое копирование зеркала между ОС (через облачные папки, `scp`/`rsync`, сетевые шары) может привести к невалидным именам или к тому, что утилита перестанет узнавать уже синхронизированные страницы.
...
В папке [`docs/ai-rules/`](docs/ai-rules/) лежит файл [`local-mirror-format.mdc`](docs/ai-rules/local-mirror-format.mdc) — описание структуры и формата локального зеркала Confluence, которое можно подключить к вашему AI-ассистенту (Cursor, Claude Code, Continue, Aider, Windsurf и др.), чтобы он корректно понимал иерархию папок, формат `index.html` (Confluence Storage Format), маркеры `.id*` и вложения при работе с выгруженным деревом страниц.
...
Отдельным разделом отчёта выводятся **несинхронизированные вложения** — файлы,
которые не удалось скачать или загрузить (например, сервер потерял двоичное
содержимое и отвечает ошибкой на ссылку скачивания), а также страницы, для
которых не удалось получить список вложений. Это расхождение зеркала, поэтому
предупреждение о таких вложениях выводится и без `--report`.
...
- **Локальное перемещение папки** (новая родительская папка имеет свой `.id`-маркер) — выполняется перемещение страницы на сервере (`ancestors`) в одном вызове API.
- **Локальное переименование папки** (имя папки разошлось с заголовком в `.id`-маркере) — страница переименовывается на сервере, даже если тело страницы не менялось. Если страницу переименовали и локально, и на сервере, она попадает в конфликты.
- **Новая папка внутри уже синхронизированного поддерева** — создаётся как страница на сервере с правильным родителем и с автоматической записью локального `.id`-маркера. Использовать `upload create` для одиночных новых страниц внутри существующей иерархии не нужно.

Если локальное тело страницы не менялось с последней синхронизации, перемещение и переименование применяются поверх серверного тела — серверные правки контента не затираются и подтянутся следующим `download merge`. Если же неизменность тела доказать нельзя, а контент на сервере новее, структурное изменение откладывается: страница попадает в раздел «Не применены локальные изменения» (`unappliedPages`, поднимает `hasIssues`) с подсказкой выполнить `download merge`, при необходимости заново переименовать/переместить папку и повторить `upload merge`. В этот же раздел попадают страницы, запись которых на сервер завершилась ошибкой.
